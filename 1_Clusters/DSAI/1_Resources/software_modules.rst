Available Software Modules
==========================

Last updated: **2025-12-02 10:52:34**

.. list-table::
   :widths: 30 70
   :header-rows: 1

   * - Module
     - Versions
   * - ATK
     - ``2.38.0-GCCcore-13.2.0``
   * - AnyEvent
     - ``7.17``
   * - AppConfig
     - ``1.71``
   * - Armadillo
     - ``12.8.0-foss-2023b``
   * - AutoLoader
     - ``5.74``
   * - Autoconf
     - ``2.71-GCCcore-13.2.0``
   * - Automake
     - ``1.16.5-GCCcore-13.2.0``
   * - Autotools
     - ``20220317-GCCcore-13.2.0``
   * - BLIS
     - ``0.9.0-GCC-13.2.0``
   * - Babel
     - ``2.13.1``
   * - Bison
     - ``3.8.2``, ``3.8.2-GCCcore-13.2.0``
   * - Boost
     - ``1.83.0-GCC-13.2.0``, ``1.88.0-GCC-13.2.0``
   * - Bottleneck
     - ``1.3.7``
   * - Brotli
     - ``1.1.0-GCCcore-13.2.0``
   * - Brunsli
     - ``0.1-GCCcore-13.2.0``
   * - CFITSIO
     - ``4.3.1-GCCcore-13.2.0``
   * - CGI
     - ``4.59``
   * - CMake
     - ``3.27.6-GCCcore-13.2.0``
   * - CPANPLUS
     - ``0.9914``
   * - Carp
     - ``1.50``
   * - Catch2
     - ``2.13.9-GCCcore-13.2.0``
   * - Clone
     - ``0.46``
   * - Cwd
     - ``3.75``
   * - Cython
     - ``3.0.10-GCCcore-13.2.0``, ``3.0.4``
   * - DBI
     - ``1.643``
   * - DBus
     - ``1.15.8-GCCcore-13.2.0``
   * - DateTime
     - ``1.59``
   * - Doxygen
     - ``1.9.8-GCCcore-13.2.0``
   * - Eigen
     - ``3.4.0-GCCcore-13.2.0``
   * - Encode
     - ``3.19``
   * - Error
     - ``0.17029``
   * - Expect
     - ``1.35``
   * - Exporter
     - ``5.77``
   * - FFTW
     - ``3.3.10-GCC-13.2.0``
   * - FFTW.MPI
     - ``3.3.10-gompi-2023b``
   * - FLAC
     - ``1.4.3-GCCcore-13.2.0``
   * - FlexiBLAS
     - ``3.3.1-GCC-13.2.0``
   * - FriBidi
     - ``1.0.13-GCCcore-13.2.0``
   * - GCC
     - ``13.2.0``
   * - GCCcore
     - ``13.2.0``
   * - GDAL
     - ``3.9.0-foss-2023b``
   * - GEOS
     - ``3.12.1-GCC-13.2.0``
   * - GLPK
     - ``5.0-GCCcore-13.2.0``
   * - GLib
     - ``2.78.1-GCCcore-13.2.0``
   * - GMP
     - ``6.3.0-GCCcore-13.2.0``
   * - GO
     - ``0.04``
   * - GObject-Introspection
     - ``1.78.1-GCCcore-13.2.0``
   * - GSL
     - ``2.7-GCC-13.2.0``
   * - GTK3
     - ``3.24.39-GCCcore-13.2.0``
   * - Gdk-Pixbuf
     - ``2.42.10-GCCcore-13.2.0``
   * - Ghostscript
     - ``10.02.1-GCCcore-13.2.0``
   * - Git
     - ``0.42``
   * - Graph
     - ``0.9727``
   * - HDF
     - ``4.2.16-2-GCCcore-13.2.0``
   * - HDF5
     - ``1.14.3-gompi-2023b``
   * - HarfBuzz
     - ``8.2.2-GCCcore-13.2.0``
   * - Heap
     - ``0.80``
   * - ICU
     - ``74.1-GCCcore-13.2.0``
   * - ImageMagick
     - ``7.1.1-34-GCCcore-13.2.0``
   * - Imath
     - ``3.1.9-GCCcore-13.2.0``
   * - Importer
     - ``0.026``
   * - Inline
     - ``0.86``
   * - JSON
     - ``4.10``
   * - JasPer
     - ``4.0.0-GCCcore-13.2.0``
   * - Java
     - ``11.0.27``, ``17.0.15``, ``17.0.6``
   * - Jinja2
     - ``3.1.2``
   * - LAME
     - ``3.100-GCCcore-13.2.0``
   * - LERC
     - ``4.0.0-GCCcore-13.2.0``
   * - LLVM
     - ``16.0.6-GCCcore-13.2.0``
   * - LibTIFF
     - ``4.6.0-GCCcore-13.2.0``
   * - LittleCMS
     - ``2.15-GCCcore-13.2.0``
   * - M4
     - ``1.4.19``, ``1.4.19-GCCcore-13.2.0``
   * - MPFR
     - ``4.2.1-GCCcore-13.2.0``
   * - Mako
     - ``1.2.4``, ``1.2.4-GCCcore-13.2.0``
   * - MarkupSafe
     - ``2.1.3``
   * - Mesa
     - ``23.1.9-GCCcore-13.2.0``
   * - Meson
     - ``1.2.3-GCCcore-13.2.0``
   * - Moo
     - ``2.005005``
   * - Moose
     - ``2.2206``
   * - Mouse
     - ``v2.5.10``
   * - NASM
     - ``2.16.01-GCCcore-13.2.0``
   * - NLopt
     - ``2.7.1-GCCcore-13.2.0``
   * - Ninja
     - ``1.11.1-GCCcore-13.2.0``
   * - OpenBLAS
     - ``0.3.24-GCC-13.2.0``
   * - OpenEXR
     - ``3.2.0-GCCcore-13.2.0``
   * - OpenJPEG
     - ``2.5.0-GCCcore-13.2.0``
   * - OpenMPI
     - ``4.1.6-GCC-13.2.0``
   * - OpenSSL
     - ``1.1``
   * - PCRE
     - ``8.45-GCCcore-13.2.0``
   * - PCRE2
     - ``10.42-GCCcore-13.2.0``
   * - PMIx
     - ``4.2.6-GCCcore-13.2.0``
   * - PPI
     - ``1.277``
   * - PROJ
     - ``9.3.1-GCCcore-13.2.0``
   * - PadWalker
     - ``2.5``
   * - Pango
     - ``1.51.0-GCCcore-13.2.0``
   * - Perl
     - ``5.38.0-GCCcore-13.2.0``
   * - Perl-bundle-CPAN
     - ``5.38.0-GCCcore-13.2.0``
   * - PostgreSQL
     - ``16.1-GCCcore-13.2.0``
   * - PyNaCl
     - ``1.5.0``
   * - Pygments
     - ``2.16.1``
   * - Python
     - ``3.11.5-GCCcore-13.2.0``
   * - Python-bundle-PyPI
     - ``2023.10-GCCcore-13.2.0``
   * - Qhull
     - ``2020.2-GCCcore-13.2.0``
   * - R
     - ``4.4.1-gfbf-2023b``, ``4.5.1-gfbf-2023b``
   * - R6
     - ``2.6.1``
   * - RStudio-Server
     - ``2023.12.1+402-gfbf-2023b-Java-11-R-4.4.1``, ``2025.09.0+387-gfbf-2023b-Java-17-R-4.5.1``
   * - Rcpp
     - ``1.0.14``
   * - Readonly
     - ``2.05``
   * - Rust
     - ``1.73.0-GCCcore-13.2.0``
   * - SOCI
     - ``4.0.3-GCC-13.2.0``, ``4.0.3-GCC-13.2.0-Boost-1.88.0``
   * - SQLite
     - ``3.43.1-GCCcore-13.2.0``
   * - SVG
     - ``2.87``
   * - SWIG
     - ``4.1.1-GCCcore-13.2.0``
   * - ScaLAPACK
     - ``2.2.0-gompi-2023b-fb``
   * - SciPy-bundle
     - ``2023.11-gfbf-2023b``
   * - SecretStorage
     - ``3.3.3``
   * - Shell
     - ``0.73``
   * - Socket
     - ``2.037``
   * - Specio
     - ``0.48``
   * - Spiffy
     - ``0.46``
   * - Storable
     - ``3.25``
   * - Switch
     - ``2.17``
   * - Szip
     - ``2.1.1-GCCcore-13.2.0``
   * - Tcl
     - ``8.6.13-GCCcore-13.2.0``
   * - Template
     - ``3.101``
   * - Test
     - ``1.26``
   * - Throwable
     - ``1.001``
   * - Tk
     - ``8.6.13-GCCcore-13.2.0``
   * - UCC
     - ``1.2.0-GCCcore-13.2.0``
   * - UCX
     - ``1.15.0-GCCcore-13.2.0``
   * - UDUNITS
     - ``2.2.28-GCCcore-13.2.0``
   * - URI
     - ``5.21``
   * - UnZip
     - ``6.0-GCCcore-13.2.0``
   * - Want
     - ``0.29``
   * - Wayland
     - ``1.22.0-GCCcore-13.2.0``
   * - X11
     - ``20231019-GCCcore-13.2.0``
   * - XSLoader
     - ``0.24``
   * - XZ
     - ``5.4.4-GCCcore-13.2.0``
   * - Xerces-C++
     - ``3.2.5-GCCcore-13.2.0``
   * - Xvfb
     - ``21.1.9-GCCcore-13.2.0``
   * - YAML
     - ``1.30``
   * - Yasm
     - ``1.3.0-GCCcore-13.2.0``
   * - alabaster
     - ``0.7.13``
   * - aliased
     - ``0.34``
   * - anaconda3
     - ``2024.02-1``
   * - ant
     - ``1.10.12-Java-17``, ``1.10.14-Java-11``
   * - appdirs
     - ``1.4.4``
   * - arpack-ng
     - ``3.9.0``, ``3.9.0-foss-2023b``
   * - askpass
     - ``1.2.1``
   * - asn1crypto
     - ``1.5.1``
   * - assimp
     - ``5.4.0``
   * - at-spi2-atk
     - ``2.38.0``, ``2.38.0-GCCcore-13.2.0``
   * - at-spi2-core
     - ``2.50.0-GCCcore-13.2.0``, ``2.51.91``
   * - atk
     - ``2.38.0``
   * - atomicwrites
     - ``1.4.1``
   * - attrs
     - ``23.1.0``
   * - autoconf
     - ``2.72``
   * - autoconf-archive
     - ``2023.02.20``
   * - automake
     - ``1.16.5``
   * - backports.entry-points-selectable
     - ``1.2.0``
   * - backports.functools_lru_cache
     - ``1.6.6``
   * - base64enc
     - ``0.1-3``
   * - bazel
     - ``6.1.2``
   * - bc
     - ``1.07.1``
   * - bdftopcf
     - ``1.1``
   * - beniget
     - ``0.4.1``
   * - berkeley-db
     - ``18.1.40``
   * - binutils
     - ``2.40``, ``2.40-GCCcore-13.2.0``, ``2.41``
   * - bison
     - ``3.8.2``
   * - bitarray
     - ``2.8.2``
   * - bitstring
     - ``4.1.2``
   * - blist
     - ``1.3.6``
   * - boolean
     - ``0.46``
   * - boost
     - ``1.81.0``
   * - brew
     - ``1.0-10``
   * - brio
     - ``1.1.5``
   * - bslib
     - ``0.9.0``
   * - build
     - ``0.10.0``
   * - bzip2
     - ``1.0.8``, ``1.0.8-GCCcore-13.2.0``
   * - c-blosc
     - ``1.21.5``
   * - cURL
     - ``8.3.0-GCCcore-13.2.0``
   * - ca-certificates-mozilla
     - ``2023-05-30``
   * - cachecontrol
     - ``0.13.1``
   * - cachem
     - ``1.1.0``
   * - cachy
     - ``0.3.0``
   * - cairo
     - ``1.16.0``, ``1.18.0-GCCcore-13.2.0``
   * - callr
     - ``3.7.6``
   * - certifi
     - ``2023.7.22``
   * - cffi
     - ``1.15.1``, ``1.15.1-GCCcore-13.2.0``, ``1.16.0``
   * - chardet
     - ``5.2.0``
   * - charset-normalizer
     - ``3.3.1``
   * - cleo
     - ``2.0.1``
   * - cli
     - ``3.6.5``
   * - click
     - ``8.1.7``
   * - clipr
     - ``0.8.0``
   * - cloudpickle
     - ``3.0.0``
   * - cmake
     - ``3.27.9``
   * - colorama
     - ``0.4.6``
   * - commonmark
     - ``0.9.1``, ``1.9.5``
   * - constant
     - ``1.33``
   * - coreutils
     - ``9.4``
   * - cpp11
     - ``0.5.2``
   * - crashtest
     - ``0.4.1``
   * - crayon
     - ``1.5.3``
   * - credentials
     - ``2.0.2``
   * - cryptography
     - ``41.0.5-GCCcore-13.2.0``
   * - cuda
     - ``11.1.0``, ``11.2.0``, ``11.5.0``, ``11.8.0``, ``12.3.0``, ``12.6.3``, ``12.8.1``, ``13.0.2``
   * - cudnn
     - ``9.5.0.50-12``
   * - curl
     - ``6.3.0``, ``8.7.1--openssl``, ``8.8.0``
   * - dbus
     - ``1.13.6``
   * - deap
     - ``1.4.1``
   * - decorator
     - ``5.1.1``
   * - desc
     - ``1.4.3``
   * - devtools
     - ``2.4.5``
   * - diffobj
     - ``0.3.6``
   * - diffutils
     - ``3.10``, ``3.9``
   * - digest
     - ``0.6.37``
   * - distlib
     - ``0.3.7``
   * - distro
     - ``1.8.0``
   * - docbook-xml
     - ``4.5``
   * - docbook-xsl
     - ``1.79.2``
   * - docopt
     - ``0.6.2``
   * - docutils
     - ``0.20.1``
   * - doit
     - ``0.36.0``
   * - double-conversion
     - ``3.3.0``
   * - downlit
     - ``0.4.4``
   * - doxygen
     - ``1.9.8``
   * - dulwich
     - ``0.21.6``
   * - ecdsa
     - ``0.18.0``
   * - ed
     - ``1.4``
   * - editables
     - ``0.5``
   * - elfutils
     - ``0.190``
   * - ellipsis
     - ``0.3.2``
   * - emacs
     - ``29.3``, ``29.4``
   * - evaluate
     - ``1.0.4``
   * - exceptiongroup
     - ``1.1.3``
   * - execnet
     - ``2.0.2``
   * - expat
     - ``2.5.0-GCCcore-13.2.0``, ``2.6.2``, ``2.6.3``
   * - fansi
     - ``1.0.6``
   * - fastmap
     - ``1.2.0``
   * - fftw
     - ``3.3.10``
   * - filelock
     - ``3.13.0``
   * - findutils
     - ``4.9.0``
   * - fixesproto
     - ``5.0``
   * - flex
     - ``2.6.3``, ``2.6.4``, ``2.6.4-GCCcore-13.2.0``
   * - flit
     - ``3.9.0``, ``3.9.0-GCCcore-13.2.0``
   * - flit_core
     - ``3.9.0``
   * - flit_scm
     - ``1.7.0``
   * - font-util
     - ``1.4.0``
   * - fontawesome
     - ``0.5.3``
   * - fontconfig
     - ``2.14.2``, ``2.14.2-GCCcore-13.2.0``, ``2.15.0``
   * - fontsproto
     - ``2.1.3``
   * - foss
     - ``2023b``
   * - freetype
     - ``2.11.1``, ``2.13.2-GCCcore-13.2.0``, ``2.13.3``
   * - fribidi
     - ``1.0.12``
   * - fs
     - ``1.6.6``
   * - fsspec
     - ``2023.10.0``
   * - future
     - ``0.18.3``
   * - gast
     - ``0.5.4``
   * - gawk
     - ``5.3.0``
   * - gcc
     - ``12.3.0``, ``8.5.0``, ``9.3.0``
   * - gcc-runtime
     - ``12.3.0``, ``8.5.0``, ``9.3.0``
   * - gdb
     - ``14.1``
   * - gdbm
     - ``1.23``
   * - gdk-pixbuf
     - ``2.42.10``
   * - gert
     - ``2.1.5``
   * - gettext
     - ``0.22``, ``0.22-GCCcore-13.2.0``, ``0.22.5``
   * - gfbf
     - ``2023b``
   * - gh
     - ``1.5.0``
   * - ghostscript
     - ``10.0.0``
   * - giflib
     - ``5.2.1-GCCcore-13.2.0``
   * - git
     - ``2.42.0-GCCcore-13.2.0``, ``2.45.1``
   * - git-lfs
     - ``3.3.0``
   * - gitcreds
     - ``0.1.2``
   * - glib
     - ``2.78.3``
   * - glibc
     - ``2.28``
   * - glob2
     - ``0.7``
   * - glproto
     - ``1.4.17``
   * - glue
     - ``1.8.0``
   * - glx
     - ``1.4``
   * - gmake
     - ``4.4.1``
   * - gmp
     - ``6.2.1``
   * - gnuplot
     - ``6.0.0``
   * - gobject-introspection
     - ``1.78.1``
   * - gompi
     - ``2023b``
   * - googletest
     - ``1.14.0-GCCcore-13.2.0``
   * - gperf
     - ``3.1``, ``3.1-GCCcore-13.2.0``
   * - gpu_sanity
     - ``1.0``
   * - graphite2
     - ``1.3.14``
   * - graphviz
     - ``8.0.5``
   * - groff
     - ``1.23.0-GCCcore-13.2.0``
   * - gsl
     - ``2.7.1``
   * - gtkplus
     - ``3.24.41``
   * - gzip
     - ``1.13-GCCcore-13.2.0``
   * - harfbuzz
     - ``10.0.1``, ``7.3.0``
   * - hatch-requirements-txt
     - ``0.4.1``
   * - hatch_fancy_pypi_readme
     - ``23.1.0``
   * - hatch_vcs
     - ``0.3.0``
   * - hatchling
     - ``1.18.0``, ``1.18.0-GCCcore-13.2.0``
   * - hdf
     - ``4.2.15``
   * - hdf5
     - ``1.14.3``
   * - help2man
     - ``1.49.3-GCCcore-13.2.0``
   * - helpers
     - ``0.1.1``
   * - highr
     - ``0.11``
   * - html5lib
     - ``1.1``
   * - htmltools
     - ``0.5.8.1``
   * - htmlwidgets
     - ``1.6.4``
   * - httpuv
     - ``1.6.16``
   * - httr
     - ``1.4.7``
   * - httr2
     - ``1.1.2``
   * - hwloc
     - ``2.9.1``, ``2.9.2-GCCcore-13.2.0``
   * - hypothesis
     - ``6.90.0-GCCcore-13.2.0``
   * - icu4c
     - ``67.1``, ``74.2``
   * - idna
     - ``3.4``
   * - if
     - ``0.0608``
   * - imagesize
     - ``1.4.1``
   * - importlib_metadata
     - ``6.8.0``
   * - importlib_resources
     - ``6.1.0``
   * - ini
     - ``0.3.1``
   * - iniconfig
     - ``2.0.0``
   * - inputproto
     - ``2.3.2``
   * - installer
     - ``0.7.0``
   * - intel-oneapi-mkl
     - ``2024.0.0``
   * - intel-oneapi-tbb
     - ``2021.12.0``
   * - intervaltree
     - ``3.1.0``
   * - intltool
     - ``0.51.0``, ``0.51.0-GCCcore-13.2.0``
   * - intreehooks
     - ``1.0``
   * - ipaddress
     - ``1.0.23``
   * - jaraco.classes
     - ``3.3.0``
   * - java
     - ``21``
   * - jbigkit
     - ``2.1-GCCcore-13.2.0``
   * - jeepney
     - ``0.8.0``
   * - joblib
     - ``1.3.2``
   * - jquerylib
     - ``0.1.4``
   * - json-c
     - ``0.17-GCCcore-13.2.0``
   * - json-glib
     - ``1.6.6``
   * - jsonlite
     - ``2.0.0``
   * - jsonschema
     - ``4.17.3``
   * - jupyterlab
     - ``3.4.5``
   * - kbproto
     - ``1.0.7``
   * - keyring
     - ``24.2.0``
   * - keyrings.alt
     - ``5.0.0``
   * - knitr
     - ``1.50``
   * - krb5
     - ``1.20.1``, ``1.21.2``
   * - later
     - ``1.4.2``
   * - lcms
     - ``2.13.1``
   * - liac-arff
     - ``2.5.0``
   * - libGLU
     - ``9.0.3-GCCcore-13.2.0``
   * - libarchive
     - ``3.7.1``, ``3.7.2-GCCcore-13.2.0``
   * - libbsd
     - ``0.12.1``, ``0.12.2``
   * - libcerf
     - ``1.3``
   * - libde265
     - ``1.0.15-GCCcore-13.2.0``
   * - libdeflate
     - ``1.19-GCCcore-13.2.0``
   * - libdrm
     - ``2.4.117-GCCcore-13.2.0``
   * - libedit
     - ``3.1-20230828``
   * - libepoxy
     - ``1.4.3``, ``1.5.10-GCCcore-13.2.0``
   * - libevent
     - ``2.1.12``, ``2.1.12-GCCcore-13.2.0``
   * - libfabric
     - ``1.19.0-GCCcore-13.2.0``
   * - libffi
     - ``3.4.4-GCCcore-13.2.0``, ``3.4.6``
   * - libfontenc
     - ``1.1.8``
   * - libgd
     - ``2.3.3``
   * - libgeotiff
     - ``1.7.3-GCCcore-13.2.0``
   * - libgit2
     - ``1.7.2``, ``1.7.2-GCCcore-13.2.0``
   * - libglvnd
     - ``1.7.0-GCCcore-13.2.0``
   * - libheif
     - ``1.19.5-GCCcore-13.2.0``
   * - libice
     - ``1.1.1``
   * - libiconv
     - ``1.17``, ``1.17-GCCcore-13.2.0``
   * - libidn2
     - ``2.3.7``
   * - libjpeg-turbo
     - ``3.0.0``, ``3.0.1-GCCcore-13.2.0``, ``3.0.3``
   * - libmd
     - ``1.0.4``
   * - libmicrohttpd
     - ``0.9.50``
   * - libmng
     - ``2.0.3``
   * - libnsl
     - ``2.0.1``
   * - libogg
     - ``1.3.5-GCCcore-13.2.0``
   * - libopus
     - ``1.5.2-GCCcore-13.2.0``
   * - libpaper
     - ``1.1.28``
   * - libpciaccess
     - ``0.17``, ``0.17-GCCcore-13.2.0``
   * - libpng
     - ``1.6.39``, ``1.6.40-GCCcore-13.2.0``
   * - libpthread-stubs
     - ``0.5``
   * - libreadline
     - ``8.2-GCCcore-13.2.0``
   * - librsvg
     - ``2.56.2``
   * - libsigsegv
     - ``2.14``
   * - libsm
     - ``1.2.4``
   * - libsndfile
     - ``1.2.2-GCCcore-13.2.0``
   * - libssh2
     - ``1.11.0``
   * - libszip
     - ``2.1.1``
   * - libtiff
     - ``4.5.1``, ``4.6.0``
   * - libtirpc
     - ``1.3.3``, ``1.3.4-GCCcore-13.2.0``
   * - libtool
     - ``2.4.7``, ``2.4.7-GCCcore-13.2.0``
   * - libunistring
     - ``1.2``
   * - libunwind
     - ``1.6.2``, ``1.6.2-GCCcore-13.2.0``
   * - libvorbis
     - ``1.3.7-GCCcore-13.2.0``
   * - libx11
     - ``1.8.7``
   * - libxau
     - ``1.0.11``
   * - libxaw
     - ``1.0.15``
   * - libxcb
     - ``1.16``
   * - libxcrypt
     - ``4.4.35``
   * - libxdmcp
     - ``1.1.4``
   * - libxext
     - ``1.3.5``
   * - libxfixes
     - ``5.0.3``
   * - libxfont
     - ``1.5.4``
   * - libxft
     - ``2.3.8``
   * - libxi
     - ``1.7.10``
   * - libxkbcommon
     - ``1.5.0``
   * - libxkbfile
     - ``1.1.2``
   * - libxml2
     - ``2.10.3``, ``2.11.5-GCCcore-13.2.0``
   * - libxmu
     - ``1.1.4``
   * - libxpm
     - ``3.5.17``
   * - libxrandr
     - ``1.5.4``
   * - libxrender
     - ``0.9.11``
   * - libxslt
     - ``1.1.38-GCCcore-13.2.0``
   * - libxt
     - ``1.3.0``
   * - libxtst
     - ``1.2.4``
   * - libyaml
     - ``0.2.5``
   * - lifecycle
     - ``1.0.4``
   * - lit
     - ``18.1.7``, ``18.1.7-GCCcore-13.2.0``
   * - llvm
     - ``14.0.6``, ``16.0.2``
   * - lmod
     - ``8.7.37``, ``8.7.37-AMD``
   * - lockfile
     - ``0.12.2``
   * - lua
     - ``5.4.6``
   * - lua-luafilesystem
     - ``1.8.0``
   * - lua-luaposix
     - ``36.1``
   * - lz4
     - ``1.9.4``, ``1.9.4-GCCcore-13.2.0``
   * - lzo
     - ``2.10``
   * - m4
     - ``1.4.19``
   * - magrittr
     - ``2.0.3``
   * - make
     - ``4.4.1-GCCcore-13.2.0``
   * - markdown-it-py
     - ``3.0.0``
   * - matlab
     - ``R2024a``, ``R2025b``
   * - mdurl
     - ``0.1.2``
   * - memoise
     - ``2.0.1``
   * - mesa
     - ``23.0.3``
   * - meson
     - ``1.3.2``
   * - meson-python
     - ``0.15.0``, ``0.15.0-GCCcore-13.2.0``
   * - metis
     - ``5.1.0``
   * - mime
     - ``0.13``
   * - miniUI
     - ``0.1.2``
   * - mkfontdir
     - ``1.0.7``
   * - mkfontscale
     - ``1.2.3``
   * - mock
     - ``5.1.0``
   * - more-itertools
     - ``10.1.0``
   * - mpc
     - ``1.3.1``
   * - mpfr
     - ``4.2.0``
   * - mpmath
     - ``1.3.0``
   * - msgpack
     - ``1.0.7``
   * - nano
     - ``7.2``
   * - nasm
     - ``2.15.05``
   * - nccl
     - ``2.20.3-1``
   * - ncdu
     - ``1.19``
   * - ncurses
     - ``6.4``, ``6.4-GCCcore-13.2.0``, ``6.5``
   * - netCDF
     - ``4.9.2-gompi-2023b``
   * - netaddr
     - ``0.9.0``
   * - netcdf-c
     - ``4.9.2``
   * - netcdf-cxx4
     - ``4.3.1``
   * - netcdf-fortran
     - ``4.6.1``
   * - netifaces
     - ``0.11.0``
   * - netlib-lapack
     - ``3.11.0``
   * - netlib-scalapack
     - ``2.2.0``
   * - nettle
     - ``3.9.1-GCCcore-13.2.0``
   * - nghttp2
     - ``1.57.0``, ``1.63.0``
   * - ninja
     - ``1.11.1``
   * - nlohmann_json
     - ``3.11.3-GCCcore-13.2.0``
   * - nodejs
     - ``20.9.0-GCCcore-13.2.0``
   * - numactl
     - ``2.0.14``, ``2.0.16-GCCcore-13.2.0``
   * - numexpr
     - ``2.8.7``
   * - numpy
     - ``1.26.2``
   * - nvhpc
     - ``21.2``, ``24.3``, ``25.1``
   * - nvitop
     - ``1.5.3``
   * - openblas
     - ``0.3.25``, ``0.3.28``
   * - openjdk
     - ``11.0.20.1_1``, ``17.0.11_9``
   * - openmpi
     - ``4.1.6``, ``slurmpmi_4.1.6``
   * - openssh
     - ``9.7p1``
   * - openssl
     - ``2.3.3``, ``3.3.0``, ``3.3.1``
   * - own
     - ``0.1``
   * - packaging
     - ``23.2``
   * - pandas
     - ``2.1.3``
   * - pango
     - ``1.50.13``
   * - parallel-netcdf
     - ``1.12.3``
   * - parent
     - ``0.241``
   * - pastel
     - ``0.2.1``
   * - patchelf
     - ``0.18.0-GCCcore-13.2.0``
   * - pathlib2
     - ``2.3.7.post1``
   * - pathspec
     - ``0.11.2``
   * - pbr
     - ``5.11.1``
   * - pcre
     - ``8.45``
   * - pcre2
     - ``10.43``, ``10.44``
   * - perl
     - ``5.38.0``, ``5.40.0``
   * - perl-clone
     - ``0.46``
   * - perl-data-dumper
     - ``2.173``
   * - perl-encode-locale
     - ``1.05``
   * - perl-extutils-config
     - ``0.008``
   * - perl-extutils-helpers
     - ``0.026``
   * - perl-extutils-installpaths
     - ``0.012``
   * - perl-file-listing
     - ``6.04``
   * - perl-html-parser
     - ``3.72``
   * - perl-html-tagset
     - ``3.20``
   * - perl-http-cookies
     - ``6.10``
   * - perl-http-daemon
     - ``6.01``
   * - perl-http-date
     - ``6.02``
   * - perl-http-message
     - ``6.44``
   * - perl-http-negotiate
     - ``6.01``
   * - perl-io-html
     - ``1.004``
   * - perl-libwww-perl
     - ``6.68``
   * - perl-lwp-mediatypes
     - ``6.02``
   * - perl-module-build
     - ``0.4232``
   * - perl-module-build-tiny
     - ``0.044``
   * - perl-net-http
     - ``6.22``
   * - perl-test-needs
     - ``0.002010``
   * - perl-try-tiny
     - ``0.31``
   * - perl-uri
     - ``5.08``
   * - perl-www-robotrules
     - ``6.02``
   * - perl-xml-parser
     - ``2.46``
   * - pexpect
     - ``4.8.0``, ``4.9.0``
   * - pigz
     - ``2.8``
   * - pillar
     - ``1.10.2``
   * - pip
     - ``23.2.1``
   * - pixman
     - ``0.42.2``, ``0.42.2-GCCcore-13.2.0``
   * - pkgbuild
     - ``1.4.8``
   * - pkgconf
     - ``1.8.0``, ``2.0.3-GCCcore-13.2.0``, ``2.2.0``
   * - pkgconfig
     - ``2.0.3``
   * - pkgdown
     - ``2.1.3``
   * - pkginfo
     - ``1.9.6``
   * - pkgload
     - ``1.4.0``
   * - platformdirs
     - ``3.11.0``
   * - pluggy
     - ``1.3.0``
   * - ply
     - ``3.11``
   * - poetry
     - ``1.6.1``, ``1.6.1-GCCcore-13.2.0``
   * - poetry_core
     - ``1.7.0``
   * - poetry_plugin_export
     - ``1.5.0``
   * - pooch
     - ``1.8.0``
   * - poppler
     - ``0.79.0``
   * - poppler-data
     - ``0.4.12``
   * - praise
     - ``1.0.0``
   * - prettyunits
     - ``1.2.0``
   * - processx
     - ``3.8.6``
   * - profvis
     - ``0.4.0``
   * - promises
     - ``1.3.3``
   * - ps
     - ``1.9.1``
   * - psutil
     - ``5.9.6``
   * - ptyprocess
     - ``0.7.0``
   * - purrr
     - ``1.0.4``
   * - py
     - ``1.11.0``
   * - py-beniget
     - ``0.4.1``
   * - py-bottleneck
     - ``1.3.7``
   * - py-build
     - ``1.0.3``
   * - py-calver
     - ``2022.6.26``
   * - py-click
     - ``8.1.7``
   * - py-cloudpickle
     - ``2.2.0``
   * - py-cython
     - ``0.29.36``
   * - py-dask
     - ``2023.4.1``
   * - py-distlib
     - ``0.3.7``
   * - py-distributed
     - ``2023.4.1``
   * - py-editables
     - ``0.3``
   * - py-filelock
     - ``3.12.4``
   * - py-flit-core
     - ``3.9.0``
   * - py-fsspec
     - ``2024.2.0``
   * - py-gast
     - ``0.5.4``
   * - py-hatch-vcs
     - ``0.3.0``
   * - py-hatchling
     - ``1.21.0``
   * - py-importlib-metadata
     - ``7.0.1``
   * - py-jax
     - ``0.4.26``
   * - py-jaxlib
     - ``0.4.26``
   * - py-jinja2
     - ``3.1.2``
   * - py-joblib
     - ``1.2.0``
   * - py-llvmlite
     - ``0.41.1``
   * - py-locket
     - ``1.0.0``
   * - py-mako
     - ``1.2.4``
   * - py-markupsafe
     - ``2.1.3``
   * - py-meson-python
     - ``0.15.0``
   * - py-ml-dtypes
     - ``0.3.1``
   * - py-mpi4py
     - ``3.1.5``
   * - py-msgpack
     - ``1.0.5``
   * - py-numba
     - ``0.58.1``
   * - py-numexpr
     - ``2.8.4``
   * - py-numpy
     - ``1.26.1``
   * - py-opt-einsum
     - ``3.3.0``
   * - py-packaging
     - ``23.1``
   * - py-pandas
     - ``1.5.3``
   * - py-partd
     - ``1.4.0``
   * - py-pathspec
     - ``0.11.1``
   * - py-pip
     - ``20.2``, ``22.2.2``, ``23.1.2``
   * - py-platformdirs
     - ``3.10.0``
   * - py-pluggy
     - ``1.4.0``
   * - py-ply
     - ``3.11``
   * - py-psutil
     - ``5.9.5``
   * - py-pybind11
     - ``2.12.0``
   * - py-pyproject-hooks
     - ``1.0.0``
   * - py-pyproject-metadata
     - ``0.7.1``
   * - py-python-dateutil
     - ``2.8.2``
   * - py-pythran
     - ``0.15.0``
   * - py-pytz
     - ``2023.3``
   * - py-pyyaml
     - ``6.0``
   * - py-scikit-learn
     - ``1.3.2``
   * - py-scipy
     - ``1.11.3``
   * - py-setuptools
     - ``50.1.0``, ``68.0.0``
   * - py-setuptools-scm
     - ``8.0.4``
   * - py-six
     - ``1.16.0``
   * - py-sortedcontainers
     - ``2.4.0``
   * - py-tblib
     - ``1.6.0``
   * - py-threadpoolctl
     - ``3.1.0``
   * - py-tomli
     - ``2.0.1``
   * - py-toolz
     - ``0.12.0``
   * - py-tornado
     - ``6.1``
   * - py-trove-classifiers
     - ``2023.8.7``
   * - py-typing-extensions
     - ``4.8.0``
   * - py-urllib3
     - ``2.1.0``
   * - py-versioneer
     - ``0.28``
   * - py-virtualenv
     - ``20.24.5``
   * - py-wheel
     - ``0.41.2``
   * - py-zict
     - ``3.0.0``
   * - py-zipp
     - ``3.17.0``
   * - py_expression_eval
     - ``0.3.14``
   * - pyasn1
     - ``0.5.0``
   * - pybind11
     - ``2.11.1-GCCcore-13.2.0``
   * - pycparser
     - ``2.21``
   * - pycryptodome
     - ``3.19.0``
   * - pydevtool
     - ``0.3.0``
   * - pylev
     - ``1.4.0``
   * - pyparsing
     - ``3.1.1``
   * - pyproject-metadata
     - ``0.7.1``
   * - pyproject_hooks
     - ``1.0.0``
   * - pyrsistent
     - ``0.20.0``
   * - pytest
     - ``7.4.3``
   * - pytest-xdist
     - ``3.3.1``
   * - python
     - ``3.11.9``, ``3.13.0``, ``3.8.6``, ``3.9.15``
   * - python-dateutil
     - ``2.8.2``
   * - python-venv
     - ``1.0``
   * - pythran
     - ``0.14.0``
   * - pytoml
     - ``0.1.21``
   * - pytorch
     - ``2.5.1``
   * - pytz
     - ``2023.3.post1``
   * - qt
     - ``5.15.2``
   * - r
     - ``4.4.1``
   * - ragg
     - ``1.4.0``
   * - randrproto
     - ``1.5.0``
   * - rapidfuzz
     - ``2.15.2``
   * - rappdirs
     - ``0.3.3``
   * - rcmdcheck
     - ``1.4.0``
   * - re2c
     - ``2.2``
   * - readline
     - ``8.2``
   * - recordproto
     - ``1.14.2``
   * - regex
     - ``2023.10.3``
   * - rematch2
     - ``2.1.2``
   * - remotes
     - ``2.5.0``
   * - renderproto
     - ``0.11.1``
   * - requests
     - ``2.31.0``
   * - requests-toolbelt
     - ``1.0.0``
   * - rich
     - ``13.6.0``
   * - rich-click
     - ``1.7.0``
   * - rlang
     - ``1.1.6``
   * - rmarkdown
     - ``2.29``
   * - roxygen2
     - ``7.3.2``
   * - rpcsvc-proto
     - ``1.4.3``
   * - rprojroot
     - ``2.0.4``
   * - rstudioapi
     - ``0.17.1``
   * - rust
     - ``1.78.0``
   * - rust-bootstrap
     - ``1.78.0``
   * - rversions
     - ``2.1.2``
   * - sass
     - ``0.4.10``
   * - scandir
     - ``1.10.0``
   * - scikit-build
     - ``0.17.6-GCCcore-13.2.0``
   * - scikit_build
     - ``0.17.6``
   * - scipy
     - ``1.11.4``
   * - sed
     - ``4.9``
   * - semantic_version
     - ``2.10.0``
   * - sessioninfo
     - ``1.2.3``
   * - setuptools
     - ``68.2.2``
   * - setuptools-rust
     - ``1.8.0``, ``1.8.0-GCCcore-13.2.0``
   * - setuptools-scm
     - ``8.0.4``
   * - shared-mime-info
     - ``1.10``
   * - shellingham
     - ``1.5.4``
   * - shiny
     - ``1.10.0``
   * - simplegeneric
     - ``0.8.1``
   * - simplejson
     - ``3.19.2``
   * - six
     - ``1.16.0``
   * - slurm
     - ``23-02-6-1``
   * - snappy
     - ``1.1.10``
   * - snowballstemmer
     - ``2.2.0``
   * - sortedcontainers
     - ``2.4.0``
   * - source-highlight
     - ``3.1.9``
   * - sourcetools
     - ``0.1.7-1``
   * - sphinx
     - ``7.2.6``
   * - sphinx-bootstrap-theme
     - ``0.8.1``
   * - sphinxcontrib-jsmath
     - ``1.0.1``
   * - sphinxcontrib_applehelp
     - ``1.0.7``
   * - sphinxcontrib_devhelp
     - ``1.0.5``
   * - sphinxcontrib_htmlhelp
     - ``2.0.4``
   * - sphinxcontrib_qthelp
     - ``1.0.6``
   * - sphinxcontrib_serializinghtml
     - ``1.1.9``
   * - sphinxcontrib_websupport
     - ``1.2.6``
   * - sqlite
     - ``3.43.2``, ``3.46.0``
   * - standard
     - ``2024.10``
   * - strictures
     - ``2.000006``
   * - stringi
     - ``1.8.7``
   * - stringr
     - ``1.5.1``
   * - swig
     - ``4.0.2-fortran``, ``4.1.1``
   * - sys
     - ``3.4.3``
   * - systemfonts
     - ``1.2.3``
   * - tabulate
     - ``0.9.0``
   * - tar
     - ``1.34``
   * - tcl
     - ``8.6.12``
   * - teckit
     - ``2.5.11``
   * - testthat
     - ``3.2.3``
   * - texinfo
     - ``7.0.3``, ``7.1``
   * - texlive
     - ``20220321``
   * - textshaping
     - ``1.0.1``
   * - threadpoolctl
     - ``3.2.0``
   * - threads
     - ``2.21``
   * - tibble
     - ``3.3.0``
   * - tinytex
     - ``0.57``
   * - toml
     - ``0.10.2``
   * - tomli
     - ``2.0.1``
   * - tomli_w
     - ``1.0.0``
   * - tomlkit
     - ``0.12.1``
   * - trove_classifiers
     - ``2023.10.18``
   * - typing_extensions
     - ``4.8.0``
   * - tzdata
     - ``2023.3``
   * - ujson
     - ``5.8.0``
   * - unzip
     - ``6.0``
   * - urlchecker
     - ``1.0.1``
   * - urllib3
     - ``2.0.7``
   * - usethis
     - ``3.1.0``
   * - utf8
     - ``1.2.6``
   * - util-linux
     - ``2.39-GCCcore-13.2.0``
   * - util-linux-uuid
     - ``2.38.1``, ``2.40.2``
   * - util-macros
     - ``1.19.3``
   * - vctrs
     - ``0.6.5``
   * - version
     - ``0.9930``
   * - versioneer
     - ``0.29``
   * - virtualenv
     - ``20.24.6``, ``20.24.6-GCCcore-13.2.0``
   * - vscodeserver
     - ``4.96.4``
   * - waldo
     - ``0.6.1``
   * - wcwidth
     - ``0.2.8``
   * - webencodings
     - ``0.5.1``
   * - wheel
     - ``0.41.2``
   * - which
     - ``2.21``
   * - whisker
     - ``0.4.1``
   * - withr
     - ``3.0.2``
   * - x265
     - ``3.5-GCCcore-13.2.0``
   * - xcb-proto
     - ``1.16.0``
   * - xcb-util
     - ``0.4.1``
   * - xcb-util-image
     - ``0.4.1``
   * - xcb-util-keysyms
     - ``0.4.1``
   * - xcb-util-renderutil
     - ``0.3.10``
   * - xcb-util-wm
     - ``0.4.2``
   * - xextproto
     - ``7.3.0``
   * - xfun
     - ``0.52``
   * - xkbcomp
     - ``1.4.6``
   * - xkbdata
     - ``1.0.1``
   * - xlrd
     - ``2.0.1``
   * - xml2
     - ``1.3.8``
   * - xopen
     - ``1.0.1``
   * - xorg-macros
     - ``1.20.0-GCCcore-13.2.0``
   * - xproto
     - ``7.0.31``
   * - xrandr
     - ``1.5.2``
   * - xtable
     - ``1.8-4``
   * - xtrans
     - ``1.5.0``
   * - xz
     - ``5.4.6``
   * - yaml
     - ``2.3.10``
   * - yaml-cpp
     - ``0.8.0-GCCcore-13.2.0``
   * - zip
     - ``2.3.3``, ``3.0``
   * - zipfile36
     - ``0.1.3``
   * - zipp
     - ``3.17.0``
   * - zlib
     - ``1.2.13``, ``1.2.13-GCCcore-13.2.0``, ``1.3.1``
   * - zlib-ng
     - ``2.1.6``, ``2.2.1``
   * - zstd
     - ``1.5.5-GCCcore-13.2.0``, ``1.5.6``
   * - zziplib
     - ``0.13.72``
